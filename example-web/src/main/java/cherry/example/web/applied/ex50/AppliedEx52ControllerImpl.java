...
import static cherry.example.web.PathDef.VIEW_APPLIED_EX52_START;
...
public class AppliedEx52ControllerImpl implements AppliedEx52Controller {
	...
	@Override
	public ModelAndView start(AppliedEx52Form form, BindingResult binding, Authentication auth, Locale locale,
			SitePreference sitePref, NativeWebRequest request) {
		return renderStartView().build();
	}

	@Override
	public ModelAndView confirm(AppliedEx52Form form, BindingResult binding, Authentication auth, Locale locale,
			SitePreference sitePref, NativeWebRequest request) {
	...
	@Override
	public ModelAndView back(AppliedEx52Form form, BindingResult binding, Authentication auth, Locale locale,
			SitePreference sitePref, NativeWebRequest request) {
		return renderStartView().build();
	}

	@Override
	public ModelAndView execute(AppliedEx52Form form, BindingResult binding, Authentication auth, Locale locale,
			SitePreference sitePref, NativeWebRequest request) {
	...
	private ModelAndViewBuilder renderStartView() {
		return ModelAndViewBuilder.withViewname(VIEW_APPLIED_EX52_START);
	}
	...
	private UriComponents redirectOnInit(String redir) {
		if (StringUtils.isNotEmpty(redir)) {
			...
			return fromMethodCall(on(AppliedEx51Controller.class).update(null, null, null, null, null, null)).build();
		}
	}

	private UriComponents redirectOnExecute(AppliedEx52Form form) {
		UriComponentsBuilder ucb = fromMethodCall(on(AppliedEx51Controller.class).update(null, null, null, null, null,
				null));
	...
	private boolean hasErrors(AppliedEx52Form form, BindingResult binding) {
		...
		if (form.getDt() == null && form.getTm() != null) {
			LogicalErrorUtil.rejectValue(binding, "dt", LogicalError.RequiredWhen,
					LogicalErrorUtil.resolve("appliedEx51Form.dt"), LogicalErrorUtil.resolve("appliedEx51Form.tm"));
		}
