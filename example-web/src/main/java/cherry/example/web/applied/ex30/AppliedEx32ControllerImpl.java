...
import cherry.example.web.applied.ex30.AppliedEx31SubFormBase.Prop;
import cherry.example.web.util.ViewNameUtil;
import cherry.foundation.logicalerror.LogicalErrorUtil;

@Controller
public class AppliedEx32ControllerImpl implements AppliedEx32Controller {

	private final String viewnameOfStart = ViewNameUtil.fromMethodCall(on(AppliedEx32Controller.class).start(0L, 0,
	...
	@Override
	public ModelAndView confirm(long id, int rownum, AppliedEx32Form form, BindingResult binding, Authentication auth,
			Locale locale, SitePreference sitePref, NativeWebRequest request) {
	...
	@Override
	public ModelAndView back(long id, int rownum, AppliedEx32Form form, BindingResult binding, Authentication auth,
			Locale locale, SitePreference sitePref, NativeWebRequest request) {
		return withViewname(viewnameOfStart).build();
	}

	@Override
	public ModelAndView execute(long id, int rownum, AppliedEx31Form form, BindingResult binding, Authentication auth,
			Locale locale, SitePreference sitePref, NativeWebRequest request) {
	...
	private UriComponents redirectOnInit(String redir, long id, int rownum) {
		if (StringUtils.isNotEmpty(redir)) {
			...
			return fromMethodCall(on(AppliedEx32Controller.class).start(id, rownum, null, null, null, null, null, null))
					.replaceQueryParam(REQ_ID, id).replaceQueryParam(REQ_ROWNUM, rownum).build();
		}
	}

	private UriComponents redirectOnExecute(long id) {
		return fromMethodCall(on(AppliedEx31Controller.class).update(id, null, null, null, null, null, null))
