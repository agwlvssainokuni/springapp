...
@Service
public class BasicEx60ServiceImpl implements BasicEx60Service {
	...
	@Transactional
	@Override
	public PagedList<BExTbl1> search(BasicEx60Form form) {
		return queryDslSupport.search(commonClause(form), orderByClause(form), form.getPno(), form.getPsz(), et1);
	}

	@Transactional
	@Override
	public void downloadXlsx(BasicEx60Form form, HttpServletResponse response) {
	...
	private QueryConfigurer orderByClause(final BasicEx60Form form) {
		return new QueryConfigurer() {
			@Override
			public SQLQuery configure(SQLQuery query) {
