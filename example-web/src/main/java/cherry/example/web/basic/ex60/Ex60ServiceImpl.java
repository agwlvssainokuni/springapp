...
@Service
public class Ex60ServiceImpl implements Ex60Service {
	...
	@Transactional
	@Override
	public PagedList<BExTbl1> search(Ex60Form form) {
		return queryDslSupport.search(commonClause(form), orderByClause(form), form.getPno(), form.getPsz(), et1);
	}

	@Transactional
	@Override
	public void downloadXlsx(Ex60Form form, HttpServletResponse response) {
	...
	private QueryConfigurer commonClause(final Ex60Form form) {
	...
	private QueryConfigurer orderByClause(final Ex60Form form) {
		return new QueryConfigurer() {
			@Override
			public SQLQuery configure(SQLQuery query) {
