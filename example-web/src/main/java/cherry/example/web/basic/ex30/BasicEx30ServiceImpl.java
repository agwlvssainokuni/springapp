...
@Service
public class BasicEx30ServiceImpl implements BasicEx30Service {
	...
	@Transactional
	@Override
	public PagedList<BExTbl1> search(BasicEx30Form form) {
		return queryDslSupport.search(commonClause(form), orderByClause(form), form.getPno(), form.getPsz(), et1);
	}

	@Transactional
	@Override
	public void downloadXlsx(BasicEx30Form form, HttpServletResponse response) {
	...
	private QueryConfigurer commonClause(final BasicEx30Form form) {
	...
	private QueryConfigurer orderByClause(final BasicEx30Form form) {
		return new QueryConfigurer() {
			@Override
			public SQLQuery configure(SQLQuery query) {
