...
package cherry.example.web.form.ex40;
...
@Controller
public class FormEx40ControllerImpl implements FormEx40Controller {

	private final String viewnameOfStart = ViewNameUtil.fromMethodCall(on(FormEx40Controller.class).start(null, null,
			null, null, null, null));
	...
	@Override
	public ModelAndView start(FormEx40Form form, BindingResult binding, Authentication auth, Locale locale,
			SitePreference sitePref, NativeWebRequest request) {
		return withViewname(viewnameOfStart).build();
	}

	@Override
	public ModelAndView confirm(FormEx40Form form, BindingResult binding, Authentication auth, Locale locale,
			SitePreference sitePref, NativeWebRequest request) {
	...
	@Override
	public ModelAndView back(FormEx40Form form, BindingResult binding, Authentication auth, Locale locale,
			SitePreference sitePref, NativeWebRequest request) {
		return withViewname(viewnameOfStart).build();
	}
