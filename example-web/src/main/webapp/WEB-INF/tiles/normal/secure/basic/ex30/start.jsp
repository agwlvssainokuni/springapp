...
<h2 class="page-header">基本画面遷移: 検索一覧系1-検索条件結果一覧</h2>
					...
					<div class="col-md-3">
						<s:nestedPath path="ex30Form">
							<ex:pageSorter cssClass="ex30Sorter" sortOrder="sort1.order" sortBy="sort1.by"
								sortByItems="${foundation:getLabeledEnumCodeList('cherry.example.web.basic.ex30.SortBy')}" />
					...
					<tbody>
						<c:forEach var="count" begin="1" end="${pagedList.list.size()}">
							<s:nestedPath path="pagedList.list[${count-1}]">
								<tr>
									<td><s:bind path="id">
											<a href="${baseUri}/basic/ex31/?id=${status.value}"><span class="glyphicon glyphicon-edit"></span>変更</a>
										</s:bind></td>
