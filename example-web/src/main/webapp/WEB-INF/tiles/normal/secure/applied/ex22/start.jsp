...
<h2 class="page-header">応用画面遷移: 単票入力系2-変更入力</h2>
<div class="panel-body">
	...
	<f:form servletRelativeAction="/secure/applied/ex22/confirm?id=${param.id}" method="POST" modelAttribute="appliedEx20Form"
			...
			<tbody>
				<c:forEach var="count" begin="1" end="${appliedEx20Form.item.size()}">
					<s:nestedPath path="item[${count-1}]">
						<tr>
							<td><a href="${baseUri}/applied/ex23/?id=${param.id}&rownum=${count-1}" class="text-nowrap"><span
				...
				<tr>
					<td><a href="${baseUri}/applied/ex23/?id=${param.id}&rownum=${appliedEx20Form.item == null ? 0 : appliedEx20Form.item.size()}"
