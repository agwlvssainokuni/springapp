...
public class WorkdayStoreImpl implements WorkdayStore {
	...
	public int getNumberOfWorkday(String name, LocalDate from, LocalDate to) {
		// SELECT COUNT(H0.dt) FROM dayoff_master H0
		// WHERE
		// H0.name = :name
		// AND
	...
	public LocalDate getNextWorkday(String name, LocalDate from, int numberOfWorkday) {
		...
		// MIN(D.dt)
		// FROM
		// (SELECT A.d*10+B.d AS n, ADD_DAYS(:from, A.d*10+B.d) AS dt FROM digit A, digit B) D
		...
		// H0.deleted_flg = 0
		// WHERE
		// NOT EXISTS (SELECT 1 FROM dayoff_master H1 WHERE H1.name = :name AND H1.dt = D.dt AND H1.deleted_flg = 0)
		// GROUP BY
		// D.n
		// HAVING
		// COUNT(H0.dt) = D.n - :n + 1
