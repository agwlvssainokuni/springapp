...
public interface SqlLoadIdController {
	...
	@ModelAttribute()
	SqlLoadForm getForm(@PathVariable(ParamDef.PATHVAR_ID) int id);
	...
	ModelAndView start(@PathVariable(ParamDef.PATHVAR_ID) int id, Authentication auth, Locale locale,
			SitePreference sitePref, HttpServletRequest request);

	@RequestMapping(PathDef.SUBURI_EXECUTE)
	ModelAndView execute(@PathVariable(ParamDef.PATHVAR_ID) int id, @Validated(G3.class) SqlLoadForm form,
	...
	ModelAndView update(@PathVariable(ParamDef.PATHVAR_ID) int id, @Validated(G2.class) SqlLoadForm form,
			BindingResult binding, Authentication auth, Locale locale, SitePreference sitePref,
