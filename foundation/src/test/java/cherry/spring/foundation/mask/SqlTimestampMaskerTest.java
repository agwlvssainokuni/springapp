...
package cherry.spring.foundation.mask;
...
import cherry.spring.foundation.mask.SqlTimestampMasker;

public class SqlTimestampMaskerTest {

	@Test
	public void testMaskYear() {
		SqlTimestampMasker masker = SqlTimestampMasker.newMasker(
		...
		assertThat(
				...
				is(new LocalDateTime(1970, v.getMonthOfYear(), v
						...
						v.getMinuteOfHour(), v.getSecondOfMinute()).toDate()
						.getTime()));
	}
	...
	public void testMaskMonth() {
		...
		assertThat(
				...
				is(new LocalDateTime(v.getYear(), 1, v.getDayOfMonth(), v
						...
						.getSecondOfMinute()).toDate().getTime()));
	}
	...
	public void testMaskHour() {
		...
		Timestamp vv = new Timestamp(v.toDate().getTime());
		assertThat(masker.mask(null), is(nullValue()));
		assertThat(masker.mask(vv).getTime(), is(new LocalDateTime(v.getYear(),
				v.getMonthOfYear(), v.getDayOfMonth(), 0, v.getMinuteOfHour(),
				v.getSecondOfMinute()).toDate().getTime()));
	}
	...
	public void testMaskSecond() {
		...
		assertThat(
				...
				is(new LocalDateTime(v.getYear(), v.getMonthOfYear(), v
						.getDayOfMonth(), v.getHourOfDay(),
						v.getMinuteOfHour(), 0).toDate().getTime()));
	}
